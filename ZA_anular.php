...
<?php
	if(isset($_POST['id_venta'])){
		...
		$ahora=date("Y-m-d");
		$observaciones.=' Se Anulo el pedido el: ' . $ahora;
		$xxx=mysqli_real_escape_string($conexion, $_POST['observaciones']);
		if($xxx<>''){
			$observaciones.=", " . mysqli_real_escape_string($conexion, $_POST['observaciones']);
		}
		M_ventas_U_id_anular($conexion, $id_venta_form, $observaciones);
		...
		$respuesta="
			<b><<< ADMINISTRADOR >>></b>
			<br>Se ha anulado un Pedido con EXITO.
		...
		$fecha_respuesta= date("Y-m-d h:m:s");
		$respuesta="
			<br>Se ha anulado un Pedido a su nombre...
			<br><b>Datos del Pedido:</b>
			<br>Pedido N°: SSPI-" . $nun . "
			<br>Vendedor: " . $datos_del_vendedor['NOMBRE'][0] . " " . $datos_del_vendedor['APELLIDO'][0] . "
			<br>Cliente: " . $nombre_cliente . "
			...
			<br>Información adicional del Pago: " . $datos_de_la_venta['ABONO_1_INF'][0] . "
			<br>Observaciones: " . $datos_de_la_venta['OBSERVACIONES'][0] . "
			<br>Puedes verificar esta información ingresando en tu sesión de usuario visitando <a href='https://www.sersuprinca.com'>nuestro sitio web</a> o contactando a uno de nuestros representantes de venta para mas detalles.
		";
		M_mensajes_enviar_correo($nombre_dest, $correo_dest, $fecha_respuesta, $respuesta);
...
<head>
	...
	<title>Anular Compra en linea</title>
</head>
<body class="bg-light">
	...
	<section class="my-5 bg-light">
		...
		?>
			<h3 class="col-12 text-center text-light mb-3"><b>Anular Compra de Cliente en linea</b></h3>
			<h6 class="col-12 text-left text-light mb-3">¿Seguro que desea Anular la compra aociada al  pedido SSP-<?php echo $numero; ?>?</h6>
			<form action="ZA_anular.php" method="post" class="row">
				...
				<div class="m-auto">
					...
					<input type="submit" value="Anular &raquo;" class="btn btn-naranja text-light mb-2 border border-dark">
				</div>
			</form>
		</div>
	</section>
