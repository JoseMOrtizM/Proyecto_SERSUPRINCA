...
<?php
	...
	if(isset($_POST['FORM'])){
		if($_POST['FORM']=='INSERTAR'){
			...
			//esto es un RD no lleva accion de modificar
		}else if($_POST['FORM']=='BORRAR'){
			$id=mysqli_real_escape_string($conexion, $_POST['id']);
			M_carrito_actualizar_producto_borrado($conexion, $id);
		}
	}
?>
<!doctype html>
...
<head>
	...
	<title>BD-Carrito de la Compra</title>
</head>
<body>
	...
	?>
			<div class="col-md-12 col-lg-10 col-xl-9 mx-auto bg-naranja border border-dark">
				<div class="row mt-4 align-items-center rounded-top px-2">
					<div class="col-md-9 mb-1 mt-3">
						<h3 class="text-center text-md-left text-light"><b>Insertar Renglon:</b></h3>
					</div>
					<div class="col-md-3 text-center text-md-right mb-1 mt-3">
						<a href="ZA_RD_carrito_de_compras.php" class="btn btn-naranja text-light mb-2 border border-dark"><span class="fa fa-reply-all"></span> Volver</a>
					</div>
				</div>
				<div class="row mt-4 align-items-center rounded-top px-2">
		...
		<div class="col-md-12 col-lg-9 mx-auto border border-dark bg-naranja">
			<form action="ZA_RD_carrito_de_compras.php" method="post" class="text-center p-2 rounded">
				...
				<div class="m-auto">
					<a href="ZA_RD_carrito_de_compras.php" class="btn btn-naranja text-light mb-2 border border-dark"><span class="fa fa-reply-all"></span> Volver</a>&nbsp;&nbsp;<input type="submit" value="Borrar &raquo;" class="btn btn-naranja text-light mb-2 border border-dark">
				</div>
			</form>
		</div>
		<br><br><br><br><br><br><br><br>
		...
		?>
		<META HTTP-EQUIV="Refresh" CONTENT="0; URL=ZA_RD_carrito_de_compras.php">
	<?php
	...
	<div class="card mb-3 bg-naranja rounded-0 col-12 col-lg-9 mx-auto px-0 text-light border border-dark">
		<div class="card-header text-center text-light">
			<h3 class='text-center'><span class="fa fa-database"></span> Carritos:</h3>
		</div>
			...
			<div class="table-responsive">
				<table class="table table-bordered table-striped table-hover TablaDinamica">
					...
					<tbody>
						...
						$datos=M_carrito_compra_R($conexion, '', '', '', '', '', '', '', '', '');
						$i=0;
						while(isset($datos['ID_CARRITO_COMPRA'][$i])){
							if($datos['ID_CARRITO_COMPRA'][$i]<>""){
								echo "<tr>";
								echo "<td class='text-left'><i><b>Cliente: </b>" . $datos['CORREO'][$i] . "<br><b>Producto: </b> " . $datos['NOMBRE_PRODUCTO'][$i] . "<br><b>Cantidad: </b> " . $datos['CANTIDAD'][$i] . " " . $datos['UNIDAD_DE_VENTA'][$i] . "<br><b>Estatus: </b> " . $datos['ESTATUS'][$i] . "</i></td>";
								echo "<td class='text-center h5'><a title='Borrar' href='ZA_RD_carrito_de_compras.php?accion=borrar&NA_Id=" . $datos['ID_CARRITO_COMPRA'][$i] . "' class='btn btn-transparent text-danger fa fa-trash-o d-inline'></a></td>";
								echo "</tr>";
							}
							$i=$i+1;
						}
